      ...
      tabItem(tabName = "simlens",
        ...
        fluidRow(
          box(
            ...
            selectInput("sim_dis_view_period", "View outputs as:", choices = c("Quarterly" = "quarterly", "Annualized" = "annual"), selected = "quarterly"),
            br(),
...
server <- function(input, output, session) {
  ...
  sim_dis_results <- reactive({
    ...
    isolate({
      ...
      per_quarter_scale <- 1 / max(input$sim_dis_horizon_q, 1)
      annual_scale <- 4 / max(input$sim_dis_horizon_q, 1)
      view_scale <- if (identical(input$sim_dis_view_period, "annual")) annual_scale else per_quarter_scale
      view_label <- if (identical(input$sim_dis_view_period, "annual")) "annualized" else "quarterly"
  ...
  output$sim_dis_summary <- renderUI({
    ...
    HTML(paste0(
      ...
      "<hr>",
      "<p><strong>Viewing mode:</strong> ", stringr::str_to_title(results$view_label), " (horizon=", input$sim_dis_horizon_q, " quarter(s))</p>",
